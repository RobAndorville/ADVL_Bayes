    ...
    Property TableName As String
        ...
        Set(value As String)
            ...
            If Main.BayesSim.Data.Tables.Count > 0 Then
                NRows = Main.BayesSim.Data.Tables(TableName).Rows.Count
            End If
            UpdateTable()
        End Set
    End Property
...
#Region " Form Display Methods - Code used to display this form." '============================================================================================================================
    ...
    Private Sub Form_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        txtMinFormat.Text = "N2"
        txtMaxFormat.Text = "N2"
        txtSumFormat.Text = "N2"
        txtAvgFormat.Text = "N2"
        txtStdDevFormat.Text = "N2"
        txtVarFormat.Text = "N2"
...
#Region " Form Methods - The main actions performed by this form." '===========================================================================================================================
    ...
    Public Sub ShowAllFieldStats()
        ...
        Else
            ...
            For Each item In Main.BayesSim.Data.Tables(TableName).Columns
                dgvStatistics.Rows.Add()
                RowNo = dgvStatistics.Rows.Count - 1
                dgvStatistics.Rows(RowNo).Cells(0).Value = item.ColumnName
                dgvStatistics.Rows(RowNo).Cells(1).Value = Main.BayesSim.Data.Tables(TableName).Compute("Min(" & item.ColumnName & ")", "")
                dgvStatistics.Rows(RowNo).Cells(2).Value = Main.BayesSim.Data.Tables(TableName).Compute("Max(" & item.ColumnName & ")", "")
                dgvStatistics.Rows(RowNo).Cells(3).Value = Main.BayesSim.Data.Tables(TableName).Compute("Sum(" & item.ColumnName & ")", "")
                dgvStatistics.Rows(RowNo).Cells(4).Value = Main.BayesSim.Data.Tables(TableName).Compute("Avg(" & item.ColumnName & ")", "")
                ...
                ElseIf rbPopulation.Checked Then
                    ...
                    dgvStatistics.Rows(RowNo).Cells(5).Value = Math.Sqrt(Var)
                Else
                    dgvStatistics.Rows(RowNo).Cells(5).Value = Main.BayesSim.Data.Tables(TableName).Compute("StDev(" & item.ColumnName & ")", "")
                    dgvStatistics.Rows(RowNo).Cells(6).Value = Main.BayesSim.Data.Tables(TableName).Compute("Var(" & item.ColumnName & ")", "")
                End If
    ...
    Private Function Variance(ByVal ColumnName As String, ByVal IsSample As Boolean) As Double
        ...
        Dim Mean As Double = Main.BayesSim.Data.Tables(TableName).Compute("Avg(" & ColumnName & ")", "")
        'Dim Var As Double = 0
        Dim DiffSq As Double = 0

        'For Each Row As DataRow In Main.BayesSim.Data.Tables(TableName).Rows
        ...
        If IsSample Then
            For Each Row As DataRow In Main.BayesSim.Data.Tables(TableName).Rows
                DiffSq += (Row.Item(ColumnName) - Mean) ^ 2 / (NRows - 1)
            Next
        Else
            For Each Row As DataRow In Main.BayesSim.Data.Tables(TableName).Rows
                DiffSq += (Row.Item(ColumnName) - Mean) ^ 2 / NRows
            Next
        End If
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        'UpdateTable()
        TableName = cmbTableList.SelectedItem.ToString
    End Sub
    ...
    Public Sub UpdateTable()
        ...
        Else
            'Dim TableName As String = cmbTableList.SelectedItem.ToString
            Dim myTableName As String = cmbTableList.SelectedItem.ToString
            'If DataSource.Data.Tables.Contains(TableName) Then
            'If Main.BayesSim.Data.Tables.Contains(TableName) Then
            If Main.BayesSim.Data.Tables.Contains(myTableName) Then
                dgvResults.Columns.Clear()
                dgvResults.AutoGenerateColumns = True
                dgvResults.DataSource = Nothing
                'dgvResults.DataSource = DataSource.Data.Tables(TableName)
                'dgvResults.DataSource = Main.BayesSim.Data.Tables(TableName)
                dgvResults.DataSource = Main.BayesSim.Data.Tables(myTableName)
                'dgvResults.AutoGenerateColumns = True
                dgvResults.AutoResizeColumns()
                dgvResults.Update()
                dgvResults.Refresh()
